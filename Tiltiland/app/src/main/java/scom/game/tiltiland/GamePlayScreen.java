...
public class GamePlayScreen extends Screen
{
    ...
    private int ArrowX = 192;
    
    private void DrawStats()
    {
    	MeterSize = 13 + (zoo.GetPopulation()* 0.37);
    	g.drawPixmap(Assets.messages, 192, 524, 0, 128, (int)MeterSize, 21); // Draw Meter
    	g.drawPixmap(Assets.messages, ArrowX + ((int)MeterSize - 5), 524, 0, 149, 31, 21); // Draw Arrow
    	
    	if(zoo.GetState() == false)
    	{
    		g.drawPixmap(Assets.messages, 224, 544, 0, 0, 320, 64);
    	}
    	else
    	{
    		g.drawPixmap(Assets.messages, 224, 544, 0, 64, 320, 64);
    	}
